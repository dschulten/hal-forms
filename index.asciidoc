...
===== +type+
The +type+ attribute controls the data type of the +property+ value. The +type+ can also be used to determine the interface control to display for user input. This is an OPTIONAL element. If the +type+ value is not supported by the document consumer, contains a value not understood by the consumer, and/or is missing, the the document consumer SHOULD assume the +type+ attribute is set to the default value: +"text"+ and render the display input as a simple text box.

Some possible settings for the +type+ value are:
  +hidden+, 
  +text+,
  +textarea+,
  +search+,
